...
export const appRouter = router({
  site: router({
    config: publicProcedure.query(async () => {
      ...
      return { seoTitle: row?.seoTitle || "Script premium siap pakai untuk bisnis digital", seoDescription: row?.seoDescription || "Temukan script web premium yang responsif, mudah dikustomisasi, dan siap dipakai.", ads: { enabled: row?.adsEnabled === 1, client: row?.adsClient || "", slot: row?.adsSlot || "", placement: row?.adsPlacement || "top" } };
    }),
  }),
  system: systemRouter,
  setup: router({
